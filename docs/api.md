...
| POST /bookings/:id/complete | Payment boundary payment:manage | None | BookingSummary | Reconciled payment and completion proof required | Replay by key | PAYMENT_RECONCILIATION → COMPLETED |
...
| POST | `/tracking/assignments/:id/location` | Bearer; assigned team member with `assignment:team` | UUID assignment ID; `{latitude,longitude}` finite numbers in geographic bounds | `{accepted,arrived}` | 400 invalid coordinates, 403 tracking inactive, 404 missing/out-of-scope assignment | Latest team point is replaced; first geofence arrival event only | Updates team last location and may record `GPS_ARRIVED` event |
| GET | `/tracking/bookings/:id` | Bearer; owning Customer | UUID booking ID | `{active,location,etaSeconds}`; location is null before eligible live data; present location has `latitude,longitude,updatedAt,stale` | 404 missing/out-of-scope booking | Read only | None; returns current accepted team only during `TEAM_ON_THE_WAY` |
| POST | `/webhooks/payments/tap` | Public Tap callback; required `hashstring` header | Raw Tap charge JSON, JOD, signed amount/status/reference fields | Existing `{accepted,duplicate,paymentId,eventId,type}` | 401 bad signature/payload, 409 unknown/mismatched reference or state | Provider/event ID unique; replay hash checked | Server-owned payment/booking transitions, history, audit and outbox |
