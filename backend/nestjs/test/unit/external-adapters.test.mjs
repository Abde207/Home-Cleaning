...
import { FcmPushNotificationProvider, InvalidPushTokenError, PushProviderConfigurationError, RetryablePushError } from '../../dist/notifications/notification.provider.js';
...
test('Twilio Verify rejects invalid codes and provider timeouts without granting a session', async () => {
  ...
  let attempts = 0;
  const timeout = new TwilioVerifyProvider('service', 'account', 'placeholder', async () => { attempts++; throw new Error('network secret'); }, async () => {});
...
test('Tap creates a hosted JOD charge with provider idempotency reference', async () => {
  ...
  const tap = new TapPaymentProvider('sk_test_placeholder', 'https://example.test/webhook', 'https://example.test/return',
    async (_url, options) => { body = JSON.parse(options.body); return Response.json({ id: 'chg_example', status: 'INITIATED', amount: body.amount, currency: body.currency,
      reference: body.reference, transaction: { url: 'https://checkout.example.test/go' } }); });
  const result = await tap.createPayment({ paymentId: randomUUID(), attemptId: 'attempt-1', amount: '12.50', currency: 'JOD', bookingNumber: 'B-1', customerPhone: '+962790000000' });
  ...
  assert.deepEqual(body.customer.phone, { country_code: '962', number: '790000000' });
});
...
test('Tap webhook verifies hash, JOD amount and attempt reference', () => {
  ...
  const body = { object: 'charge', id: 'chg_example', status: 'CAPTURED', amount: 12.5, currency: 'JOD',
    transaction: { created: '1234567890000' }, reference: { gateway: 'gateway-1', payment: 'pay-1', transaction: 'attempt-1', order: 'B-1' } };
  ...
  assert.equal(event.bookingReference, 'B-1');
...
test('Tap timeouts and unconfirmed refunds fail without financial success', async () => {
  const input = { paymentId: randomUUID(), attemptId: randomUUID(), amount: '12.50', currency: 'JOD', bookingNumber: 'B-2', customerPhone: '+962790000000' };
  let attempts = 0;
  const unavailable = new TapPaymentProvider('sk_test_placeholder', 'https://example.test/webhook', 'https://example.test/return',
    async () => { attempts++; throw new Error('network details'); }, async () => {});
...
test('Tap retries the same idempotent refund after a transient provider failure', async () => {
  ...
  const tap = new TapPaymentProvider('sk_test_placeholder', 'https://example.test/webhook', 'https://example.test/return',
    async (_url, options) => {
      ...
      return Response.json({ id: 're_example', status: 'REFUNDED', charge_id: request.charge_id, amount: request.amount,
        currency: request.currency, reference: { idempotent: request.reference.idempotent } });
    }, async () => {});
  const result = await tap.refund({ paymentId: randomUUID(), refundId, amount: '12.50', currency: 'JOD', providerReference: 'chg_example' });
  assert.equal(calls, 2);
  assert.equal(result.providerReference, 're_example');
});
...
test('FCM classifies invalid registrations without leaking tokens', async () => {
  ...
  const fcm = new FcmPushNotificationProvider('test-project', 'test@example.test', privateKey,
    async (url) => url.includes('oauth2') ? Response.json({ access_token: 'fake-access', expires_in: 3600 }) :
...
test('FCM sends one bounded event payload and surfaces retryable provider failure', async () => {
  ...
  await assert.rejects(failure.send({ token: 'test-registration', title: 'Update', body: '', data: {} }), RetryablePushError);
  const configuration = new FcmPushNotificationProvider('test-project', 'test@example.test', privateKey,
    async (url) => url.includes('oauth2') ? Response.json({ access_token: 'fake-access' }) : new Response('', { status: 403 }));
  await assert.rejects(configuration.send({ token: 'test-registration', title: 'Update', body: '', data: {} }), PushProviderConfigurationError);
  await assert.rejects(fcm.send({ token: 'test-registration', title: 'Update', body: '', data: { 'google.bad': 'value' } }), /PUSH_PAYLOAD_INVALID/);
});
