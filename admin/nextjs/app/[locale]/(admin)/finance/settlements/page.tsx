...
import { SettlementsView } from '../../../../../components/finance';
import { ReadError, ForbiddenRead } from '../../../../../components/read-models';
import type { AdminSettlement } from '../../../../../lib/admin-api';
...
export default async function Page({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<SearchParams> }) { const { locale } = await params; if (!isLocale(locale)) redirect('/en'); if (!await mayRenderAdminPage()) return <ForbiddenRead locale={locale} />; const raw = await searchParams; const query = Object.fromEntries(Object.entries(raw).map(([key, value]) => [key, one(value)]).filter((entry): entry is [string, string] => typeof entry[1] === 'string' && entry[1].length > 0)); try { return <SettlementsView locale={locale} rows={await adminGet<AdminSettlement[]>(`/admin/settlements${queryString({ status: query.status, companyId: query.companyId, periodFrom: instant(query.periodFrom), periodTo: instant(query.periodTo) })}`)} query={raw} />; } catch (error) { return <ReadError locale={locale} error={error} />; } }
